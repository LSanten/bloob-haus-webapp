...
function renderBg(bg) {
  ...
  return `<div class="scene-nav-bg-layer" style="${layerStyle}"><img src="${escAttr(bg.image)}" alt="" aria-hidden="true" draggable="false" /></div>`;
}
...
function renderElement(el) {
  ...
  if (el.type === "image" || !el.type) {
    inner = `<img src="${escAttr(el.image)}" alt="${escAttr(el.alt || el.label || "")}" draggable="false" />${labelHtml}`;
  } else {
...
function buildContainer(backgrounds, elements, aspectRatio, maxWidth, extraClass, edgeFade, debug = false) {
...
export function render(sceneData, settings = {}) {
  ...
  if (!mobile) {
    const html = buildContainer(backgrounds, elements, aspectRatio, maxWidth, "", edgeFade, debug);
