...
| 45 | Date pills diverge: the `article` shape's is frosted glass, marbles-pouch's `.marble-date` is still a flat chip | 2026-07-28 made `.article-date` translucent for every theme that uses the `article` shape (melt, AE). marbles-pouch does not use that shape — its layouts render `.marble-date` / `.marble-date--updated`, an independent implementation with an opaque `var(--card-bg)` fill. So "date pills are glass now" is true of *articles* and false of marbles, and the two will keep drifting because nothing connects them. **Not a bug today** — marbles looks exactly as it did — it is a second implementation of one theme-standard component | `grep -n 'marble-date' themes/marbles-pouch/assets/css/main.css` (exists = still divergent), then compare a marbles page's date pill with a melt one: melt's is `backdrop-filter: blur(7px)` with a `color-mix(... transparent)` fill, marbles' is `background: var(--card-bg)` | Decide whether the date pill is a **theme standard** (each theme owns its own look — then document the divergence in `docs/architecture/theme-standards.md` and stop calling it shared) or a **shape concern** (marbles-pouch's layouts move onto `.article-date`, or the glass tokens move somewhere both can read). Cheap either way; the cost of leaving it is that the next "make the date pill X" request silently hits one theme |
| 48 | `ken-burns-zoom` ships no `manifest.json` | It is a real shape (`index.js`, `browser.js`, `styles.css`, `schema.md`) but with no manifest it has no `detect` block, and a shape without one is **always loaded** — so it silently opts out of per-page asset loading on every site that enables the feature | `ls lib/visualizers/ken-burns-zoom/manifest.json` (absent), and `npx vitest run tests/build/visualizer-discovery.test.js` pins it — that test fails if a **second** shape loses its manifest | Add a `manifest.json` with `type`, `name`, activation and a `detect.selectors` list covering the classes `styles.css` owns (ownership-based, not renderer-based — see DECISIONS 2026-07-28). Then run `scripts/audit-visualizer-detection.js` before trusting it |
...
`46` garden `src:` could not hold spaces (2026-08-15 — fixed same session it was raised) ·
