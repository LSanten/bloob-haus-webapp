...
async function generateSiteData(config) {
  ...
  const snippets = { head: "", bodyEnd: "" };
  for (const [name, target] of Object.entries(EMBED_TARGETS)) {
    if (embeds[name]) {
      snippets[target] = snippets[target] ? `${snippets[target]}\n${embeds[name]}` : embeds[name];
    }
  }
