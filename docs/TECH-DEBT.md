...
| 46 | `garden` `src:` attribute cannot hold a filename with spaces | A legal-looking fence silently loses its image — no warning, the image just goes missing | Author a garden element with `src:` pointing at a file whose name contains a space; confirm the rendered `<img>` src is truncated at the first space. `grep -n 'split(/\\s+/)' lib/visualizers/garden/parser.js` (~line 214) | Accept a quoted value (`src:"Contact us.png"`) and/or decode `%20`, matching `shapes.md` convention #4. Garden is the last shape that cannot express a spaced ref; scene-nav was fixed 2026-08-15 |
