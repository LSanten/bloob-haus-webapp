...
import { isExternalTarget, looksLikeBareDomain } from "../link-target.js";
...
export function initPanel(root, scene, { onClose } = {}) {
  ...
  function addFromFile(file, asBackground) {
    ...
    if (asBackground) {
      ...
      scene.elements.push({ type: "image", image: name, alt: name.replace(/\.[^.]+$/, ""), x: 45, y: 45, scale: 18, rotation: 0, resetRotationOnHover: true, label: false, glow: "#FFD700", glowIntensity: 1, hoverGlow: true, hoverScale: true, action: "link", value: null, newTab: null, flipH: false, flipV: false, onlyShowOn: null, mobileOverride: null });
  ...
  // ── "opens in" — the tri-state newTab control ──────────────────────────
  // Auto is the default and writes nothing to the markdown; the chip below says what
  // auto currently resolves to for whatever is typed in `goto`, so the rule is visible
  // instead of guessed at. refresh() is called from the goto field on every keystroke —
  // re-rendering the whole panel there would steal focus out of the input.
  const NEW_TAB_CHOICES = [["auto", null], ["this tab", false], ["new tab", true]];
  function newTabRows(getValue, getNewTab, setNewTab) {
    const seg = h("div", { class: "snb-seg", style: "flex:1" });
    const chip = h("div", { class: "snb-hint" });
    function refresh() {
      const nt = getNewTab();
      seg.replaceChildren(...NEW_TAB_CHOICES.map(([label, val]) =>
        h("button", { class: nt === val ? "snb-on" : "", onclick: () => { setNewTab(val); refresh(); } }, [label])
      ));
      const v = String(getValue() || "");
      let text;
      if (nt === null) {
        text = !v ? "auto — no link set yet"
          : isExternalTarget(v, window.location.origin)
            ? "auto → new tab (points off this site)"
            : "auto → this tab (points inside this site)";
      } else {
        text = nt ? "always opens a new tab" : "always opens in this tab";
      }
      if (looksLikeBareDomain(v)) text += " · looks like a domain — add https:// so it counts as external";
      chip.textContent = text;
    }
    refresh();
    return { row: h("div", { class: "snb-row" }, [h("label", {}, ["opens in"]), seg]), chip, refresh };
  }
  ...
  function renderPanel() {
    ...
    if (idx.length === 1 && primary != null) {
      ...
      // Position/size/rotation are the only per-element props that diverge on mobile
      // (writeVal/readVal target el.mobileOverride in mobile mode). Everything else —
      // glow, overlay, flip, label — is shared with desktop, so hide it in mobile mode
      // and say so, to keep 'edit mobile layout' unambiguous.
      const kids = [
        numRow("x", "x", -50, 150, 0.1), numRow("y", "y", -50, 150, 0.1),
        numRow("scale", "scale", 2, 80, 0.5), numRow("rotation", "rotation", -180, 180, 1),
      ];
      if (mobileMode) {
        kids.push(h("div", { class: "snb-notice" }, ["Editing MOBILE layout — position, size & rotation only. Glow, overlay, flip & label are shared with desktop; switch off 'edit mobile layout' (Scene panel) to change those."]));
      } else {
        kids.push(
        ...
        const nt = newTabRows(() => el.gotoRaw || el.value || "", () => el.newTab ?? null, (v) => { el.newTab = v; });
        kids.push(textRow("goto", () => (el.gotoRaw || el.value || ""), (v) => { el.gotoRaw = v; el.value = v; el.action = v.startsWith("#") ? "anchor" : "link"; nt.refresh(); }, "note.md, /url/, #anchor"));
        kids.push(nt.row, nt.chip);
      ...
      body.append(section(mobileMode ? "Selected element — MOBILE" : "Selected element", true, kids));
    } else if (idx.length > 1) {
      const kids = [
      ...
      // Bulk "opens in" — the chip previews against the primary element's own goto.
      const bulkNt = newTabRows(
        () => scene.elements[primary]?.gotoRaw || scene.elements[primary]?.value || "",
        () => scene.elements[primary]?.newTab ?? null,
        (v) => idx.forEach((i) => (scene.elements[i].newTab = v)),
      );
      kids.push(h("div", { class: "snb-hint" }, ["bulk link target (all selected)"]), bulkNt.row, bulkNt.chip);
