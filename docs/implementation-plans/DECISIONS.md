...
|------|----------|-----------|
| 2026-07-07 | **Bloob Haus owns a dedicated free Gmail (`bloobhaus.dev@gmail.com`) as the single owner identity for Google Cloud (OAuth app) + Scaleway; `admin@bloob.haus` is a Cloudflare Email Routing forward into that inbox; never tie OAuth-app/cloud-billing to a personal account. Upgrade to Workspace only at public launch if a branded consent-screen email is wanted.** | Ownership/handoff/blast-radius: the account that owns the OAuth app + cloud billing must be handoff-able and not shared-fate with personal email — but Workspace ($ ) isn't justified yet. A free Gmail gives a dedicated project inbox cleanly separated from personal; `admin@bloob.haus` (Cloudflare forward, *not* a Google account) is the public/branded address that lands in that inbox. A branded consent-screen support email would require the account to *be* `admin@bloob.haus` (loses the dedicated inbox) or a Workspace Group (paid) — and it's the *smallest* trust lever anyway (consent-screen trust = app name + verification + verified `bloob.haus` domain + minimal scopes, not the support email). So the branded email is deferred to a deliberate pre-launch verification/branding bundle. Same Gmail used for Scaleway; 2FA + recovery phone/email set on it. |
