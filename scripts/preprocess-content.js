...
import { filterPublishableFiles, removeExcludedFiles } from './utils/publish-filter.js';
import { buildFileIndex, buildAttachmentIndex } from './utils/file-index-builder.js';
import { resolveWikiLinks } from './utils/wiki-link-resolver.js';
import { resolveMarkdownLinks } from './utils/markdown-link-resolver.js';
import { resolveAttachments, copyAttachments } from './utils/attachment-resolver.js';
...
export async function preprocessContent({
  ...
  const attachmentIndex = await buildAttachmentIndex(contentDir, obsidianConfig.attachmentFolderPath);
  ...
  for (const file of published) {
    ...
    // 6a: Handle transclusions first (before other ![[]] patterns)
    ...
    // 6b: Resolve attachments (images)
    const attachmentResult = resolveAttachments(processedContent, attachmentIndex);
    ...
    const pageInfo = fileIndex.pages[Object.keys(fileIndex.pages).find(
    ...
    console.log(`[process]   → Written to: ${path.relative(ROOT_DIR, outputPath)}`);
  }
