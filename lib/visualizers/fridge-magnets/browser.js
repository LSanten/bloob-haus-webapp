...
/* ─── Parse ──────────────────────────────────────────── */
// Parses both [text] and [text](x,y) formats
function parseInput(text) {
  // Match [text] optionally followed by (x,y)
...
function addCanvas(root, preloadText = "", initialHeight = 280) {
  ...
  wrapper.querySelector(".fm-copy-btn").addEventListener("click", () => copyCanvas(id, false));
  wrapper.querySelector(".fm-copy-layout-btn").addEventListener("click", () => copyCanvas(id, true));
...
function loadCards(canvasId, initialHeight) {
  ...
  // Check if any cards have positions — if so, use positioned mode
  ...
  requestAnimationFrame(() => {
    if (hasPositions) {
      // Position cards at their specified locations
      positionCards(area, canvas.cards, initialHeight);
    } else {
      // Auto-layout cards in rows
...
function copyCanvas(canvasId, withLayout = false) {
  ...
  const text = withLayout
    ? sorted.map((c) => `[${c.text}](${Math.round(c.x)},${Math.round(c.y)})`).join(" ")
