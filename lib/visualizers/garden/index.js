...
import { encodeRef } from "../_utils/ref-encoding.js";
...
/**
 * Model ref → a URL safe to put in an `<img src>`.
 *
 * The parser stores refs DECODED (shapes.md → "Authoring & resolution conventions" #4), so
 * a filename with a space arrives here as "Contact us.png" and must be percent-encoded on
 * the way out. Encoding is per path segment, so `/` keeps separating folders.
 */
function rootPath(p) {
  if (/^https?:/.test(p)) return p;
  const encoded = encodeRef(p);
