...
|------|----------|-----------|
| 2026-07-28 | **A shape that navigates on click decides the tab from the link's ORIGIN, not from a per-shape habit: same tab for a target on this site, new tab for one off it. An explicit `newTab: on/off` overrides, and absent means nothing is written to the markdown.** The policy is a pure module with the origin **injected** (`lib/visualizers/scene-nav/link-target.js`), shared by the runtime, the builder and the tests. | scene-nav did `window.open(value, "_blank")` unconditionally, so clicking the melt homepage nav spawned a tab for a page on the same site — a site that opens a tab per click is the bug, not the feature. Origin comparison is the only rule that needs no author input and is right by default, which matters because the overwhelming majority of scene-nav links are internal. Purity with an injected origin is what stops the builder's live preview from drifting from what actually happens on click — the exact failure the pure-renderer standard exists to prevent. Two edges are decided deliberately rather than left to fall out: `mailto:`/`tel:` route as **same-tab** (the OS handler takes over and the page stays put, so a blank tab is strictly wrong), and a scheme-less `example.com` is **indistinguishable from a relative path** and therefore routes as internal — no heuristic can fix this safely, so the builder *says so* and offers the `https://` fix instead of guessing. An exported embed **inverts the default to new-tab**, because it sits on a third party's page where navigating the host away is never wanted. The GUI exposes the tri-state as auto/this-tab/new-tab with a live chip showing what auto currently resolves to, so the rule is visible rather than inferred. Convention recorded in `docs/architecture/shapes.md` → "Authoring & resolution conventions" #7. |
| 2026-07-28 | **The visitor count is rendered from GoatCounter's JSON endpoint into our own markup — never their `<img>`/iframe badge — and it must fail INVISIBLY, because the switch that most often blocks it is not in this repo.** Opt-in via `show_visitor_count` (site) / `visitor_count` (page); the site code is **derived** from the existing `goat-counter-tracking` snippet. | The badge endpoints (`.png`/`.html`) carry GoatCounter branding and a fixed 200×80 box; `.json` returns `{count}` with `Access-Control-Allow-Origin: *` (verified against a live site that has the feature enabled), so the number can inherit the theme's own type, color and glass pill like any other metadata. The load-bearing decision is the failure mode: GoatCounter gates the counter behind **"Allow using the visitor counter" in each site's own settings, which defaults to off** and answers `403 Need to enable the 'allow using the visitor counter' setting` — confirmed against both `melt` and `studiobloob`. Nothing in the build can detect that, so the widget renders `hidden` and unhides only once a count arrives; a site that flipped only the repo-side switch shows nothing rather than an empty or broken pill. Deriving the site code from the tracking snippet the user already pasted removes a second place to get it wrong; a self-hosted install has no code subdomain, yields `null`, and the counter simply stays off. Per-page override mirrors `comments:` so the two analytics/engagement switches behave identically. |
| 2026-07-28 | **`slugifyPath()` in `scripts/utils/slug-strategy.js` is THE folder-path → URL-path derivation. Every consumer goes through it — including the folder-index permalink in `preprocess-content.js` and the value interpolated into `templates/folder-index.md`.** The on-disk directory name stays raw for *filesystem* paths only. | The rule (split on `/`, slugify each segment, rejoin) already existed and agreed in three places — `file-index-builder`, the link resolver, and the Obsidian plugin's copy-link — while one place hand-rolled `"/" + dir + "/"` and skipped the strategy. That put a `case: lower` site's folder index at `/Resources/` with its children under `/resources/`. **macOS hides this class of bug completely**: a case-insensitive filesystem merges the two directories, so the site is correct locally and only splits on a Linux CI build — which is why it survived unnoticed. The same raw-name mistake appeared independently in the folder-index template's call site, where `folder=Resources` matches nothing *silently* because `graph.json`'s `section` is slugified; one slugified value fixes both, so the template's `{{ slug }}` is now always URL-space. Rule for anything new that builds a URL from a path: call `slugifyPath`, never hand-roll. Contract + implementations table: `docs/architecture/urls-and-ids.md`. |
| 2026-07-28 | **Display-name prettification has exactly one implementation, `scripts/utils/smart-title-case.js`, and it runs on the ORIGINAL filename/folder name — never on the URL slug.** Only the first letter of each word is touched; joining words stay lowercase unless they lead. | Five copies had accreted (`prettifyFolderName`, two `folderDisplay` blocks, the `titleCase` Nunjucks filter, and AE's `folderDisplayName`), and only one knew about small words — so the same folder rendered "Come To Melt" in one surface and "come-to-melt" in another. The load-bearing half of the decision is the *input*: on a `case: lower` site the slug has already destroyed the casing, so "MELT" is unrecoverable from "melt". Prettifying the original name is what lets `come-to-MELT.md` render "Come to MELT" — and it is why melt's breadcrumb resolves an ancestor folder's real page title rather than beautifying its URL segment. Touching only the leading character preserves acronyms and interior capitals (McKenzie) for free. Wiki-links are unaffected because they resolve through `filenameLookup`, still keyed on the raw on-disk filename. |
| 2026-07-28 | **The generated folder-index body comes from a markdown TEMPLATE that composes a shape** (`themes/<theme>/templates/folder-index.md`, else `themes/_base/`), not a hard-coded HTML string. Templates live in `templates/`, deliberately **not** `pages/`. Precedence: author's `_index.md`/`index.md` → theme template → `_base` template. The default declares `bloob-shape: article` + `layout: layouts/article.njk` with a nested `collection`. | Completes the `.md`-templates decision above for the last surface that still hard-coded markup: `preprocess-content.js` wrote a literal `<div class="folder-preview-visualizer">` into every generated stub, so folder indexes opted out of the shape system and duplicated markup owned by a stylesheet elsewhere. A template makes the fallback readable, theme-overridable, and consistent with what authors already write — AE's `projects/_index.md` is *exactly* `bloob-shape: article` + a `collection` fence, so generated and authored indexes are now structurally identical, and the nested collection picks up the width contract by sitting in `.article-body`. `templates/` is separate from `pages/` because Eleventy renders everything in `pages/` — a `folder-index.md` there would publish itself at `/folder-index/`. Placeholders are substituted at generation time, not by Eleventy: `graph.json`'s `section` is slugified, so an interpolated raw folder name yields `folder=Resources`, which matches nothing *silently*. **Two accepted consequences:** the listing renderer changes `folder-preview` → `collection` (melt/marbles unverified here — no vaults on this machine; to be checked after the upstream cherry-pick), and generated folder indexes become Pagefind-indexed because `article.njk` sets `data-pagefind-body` where `folder-index.njk` set `data-pagefind-ignore`. A theme keeps its own folder chrome by shipping a template override pointing at `layouts/folder-index.njk`. |
| 2026-07-28 | **Builder-owned page templates are `.md` files that COMPOSE SHAPES, not `.njk` files that emit markup.** They live in `themes/_base/pages/`, a theme overrides one by dropping a same-basename file in its own `pages/` (any extension), and `assemble-src.js` skips the shadowed base file. First case: `themes/_base/pages/tags.md`. | A `.njk` page template emits raw HTML, and every theme that had one grew a duplicate card renderer: AE's `pages/tags.njk` hand-rolled `.fp-card` markup, which then (a) drifted from the `collection` shape it was explicitly meant to match and (b) broke under per-page visualizer loading, because the markup carried no container class detection could see. A `.md` template composes shapes instead, so one renderer serves every surface and styling/detection/crawlability/SEO come for free — `/tags/<tag>/` and `/projects/` are now *the same shape*, verified structurally identical rather than by eye. Requires nothing new: `templateFormats` already includes `md` and `markdownTemplateEngine: "njk"` already interpolates `{{ }}` inside a fence. Shadowing had to become basename-based because `_base/tags.md` + a theme's `tags.njk` would otherwise both land in `src/` and fight over one `permalink:`. **Known cost:** override-by-copy drifts, so base templates are kept minimal to make copying unattractive. Migration is opt-in — marbles-pouch and warm-kitchen keep their `tags.njk` and are untouched. |
| 2026-07-28 | **Tag pages paginate `collections.tagList`, never `collections` + a hand-maintained filter list.** | The `collections` approach requires enumerating every non-tag collection by name, and per-section collections are registered *dynamically* from top-level content folders — so the list cannot be complete by construction, and each theme's had drifted to a different set. It silently fabricated a tag page for every content folder: AE shipped an empty `/tags/clients/` generated from its `clients/` folder, the one holding an unlisted client deliverable. `tagList` is already exactly the real frontmatter tags minus reserved meta-tags. Switching AE produced **zero** membership changes across all 16 real tags and removed exactly the one bogus page. |
| 2026-07-28 | **A shape's `detect.selectors` must list the classes its stylesheet OWNS, not the containers its own renderer happens to emit.** `folder-preview` now detects `.fp-cards` / `.fp-card` alongside its container classes. | Second instance of the `article`/`.share-bar` failure mode, so it is a pattern rather than a one-off: `folder-preview/styles.css` owns the `.fp-card*` grid, but AE's `pages/tags.njk` renders that markup by hand with no folder-preview container anywhere. Detection listed only containers, so enabling per-page loading dropped the stylesheet on every tag page and the 3-column grid collapsed to a full-width unstyled stack (measured `display: block`, no border, cards at 1136px). The rule that prevents recurrence is ownership-based: ask "what classes does this stylesheet style?", never "what does my renderer output?" — because a theme, a partial, or another shape may render the same markup. |
| 2026-07-28 | **`scripts/audit-visualizer-detection.js` samples one page per detection *signature*, and that sampling is unsound for theme-rendered markup — a clean audit is necessary but NOT sufficient.** | The audit passed on AE with 7/7 signatures / 86/86 pages covered while tag pages were visibly broken. Grouping by detected-shape set assumes pages sharing that set render equivalent markup; a theme template that hand-rolls another shape's classes violates the assumption, and the audit never measures the page where the markup actually differs. Until the sampler also groups by template (`inputPath`), enabling per-page loading on a site requires a manual browser check of each distinct *template*, not just a green audit. Recorded as TECH-DEBT #42. |
| 2026-07-28 | **AE keeps the pill-shaped search inputs** (`--sv-input-radius` left at its `999px` default) even though the theme sets `--border-radius: 0px`. | AE's site-wide Pagefind bar was already a 999px pill, so inheriting the pill on the collection filter input makes the two search surfaces consistent — the inconsistency was the status quo, not the change. Squaring them (setting `--sv-input-radius: var(--border-radius)`) remains a one-line theme change if that judgement is revisited. Reviewed against the live AE build 2026-07-28; closes the open question in the (now deleted) AE verification seed. |
| 2026-07-27 | **Per-page visualizer loading detects shapes from RENDERED HTML, is OPT-IN per shape (`detect` in manifest.json; unannotated shapes always load), gated per site by `features.per_page_visualizers`, and may only be enabled on a site after `scripts/audit-visualizer-detection.js` passes against a real build.** | Detecting from output rather than predicting from source means one mechanism covers every activation route — code fence, `:::` container, `bloob-shape:` frontmatter, and theme partials that drop a shape into the nav or footer — with no per-route special-casing. Opt-in is the load-bearing safety choice: a wrong guess silently strips styling from a live page, which is far worse than shipping unused KB, so the default must be "load it". The mandatory audit exists because the obvious verification is insufficient — checking "does this shape emit the class I declared?" misses that **a stylesheet can own classes something else renders**: `article/styles.css` owns `.share-bar`, which a shared theme partial renders on ordinary pages, and detecting only `.article-page` stripped it from 33 melt pages while a screenshot diff showed only an unexplained 5px height change. The audit disables exactly what detection would drop and compares element geometry; it must ignore zero-size nodes (a dropped shape's JS goes with its CSS, so its runtime-injected `display:none` scaffolding can never appear) and rAF-animated nodes (marble float otherwise reads as a regression). Enabled on melt and marbles after clean audits; **buffbaby and alter-engineers left off** because AE cannot be built on Leon's machine and enabling blind defeats the point of the flag. Doc: `docs/architecture/visualizers.md` → "Per-page asset loading". |
| 2026-07-27 | **Every content-generating shape must have a PURE `renderer.js` that produces its complete markup as a string; `browser.js` is behavior-only. External data enters through a pure `resolve.js` with the data injected. Indexability is therefore a property of the shape, not of which visual was chosen — and there is deliberately no `seo:` on/off setting.** | The requirement is a *pure renderer*, NOT that every shape run in the browser. Once the renderer is pure, running it in a browser is a free per-host choice made later, so the three hosts (Node build, standalone playground, browser fallback) differ only in where data comes from — never in what they emit. This is what the future standalone visualizer web app is built on: it gets every conforming shape for free, with no second implementation to drift. That drift is not hypothetical — scene-nav's magic machine grew its own parser/renderer copy and had to be deleted (2026-07-20 decision above). Forcing the discussion: `collection` shipped build-time HTML only for `display: cards`, so the live melt Resources page served a crawler **zero** links; investigation showed all four "runtime-only" renderers were plain `createElement` markup with behavior bolted on — the JS-only-ness was incidental, not essential. The rejected alternative was a uniform crawlable block that JS replaces: it causes a visible flash and needs `display:none` content, which Google discounts. An `seo:` toggle was rejected because hiding content from crawlers via JS is not a privacy mechanism — `visibility: unlisted` already is one. Exempt: shapes that only enhance existing text (`latex`, `citations`, `checkbox-tracker`, `page-preview`); the test is "if JS never runs, is content missing?". Standard: `docs/architecture/visualizers.md` → "The pure-renderer standard". Spec: `docs/superpowers/specs/2026-07-27-collection-pure-renderer-design.md`. Retrofit debt: TECH-DEBT #38. |
| 2026-07-27 | **Search surfaces share a `--sv-*` token contract rather than markup. Appearance tokens (`--sv-input-bg`, `--sv-input-bg-focus`, `--sv-input-focus-ring`) are CONSUMED WITH FALLBACKS and never declared in visualizer CSS, so a theme can set them from `:root` with no `!important`.** | The site-wide search bar styles Pagefind's own `.pagefind-ui__search-input`; the collection shape styles its own `<input class="fp-search-input">`. They cannot share markup, so they share variables — one token set drives both, and roundness/glass follow whatever theme is active. The consume-with-fallback rule exists because **theme `main.css` loads BEFORE visualizer CSS**: a value declared in a visualizer's `.search-visualizer{…}` block beats a theme's `:root` value at equal specificity, which is exactly why melt and marbles-pouch had accumulated `!important` overrides. Declaring nothing and reading `var(--sv-input-bg, var(--card-bg))` at point of use inverts that, letting themes win cleanly; both themes' `!important` blocks were removed and verified pixel-identical in a real browser. |
| 2026-07-21 | **A shape's debug builder is "icon-first": `debug: on` mounts a small per-scene "✎ edit" icon, and clicking it opens ONE shared sidebar bound to that scene (not an auto-opened, always-on panel). A mobile layout inherits desktop by default (divergence is opt-in) and the builder surfaces that state plainly.** | Icon-first solves three things at once: it removes the real bug that N `debug` blocks mounted N overlapping fixed panels, it keeps the page's visual layout uncluttered when several scenes are in debug, and it makes "which scene does the sidebar edit?" unambiguous (the icon is anchored to the scene, and the active scene gets an outline). On-canvas resize/rotate **grips** replaced the hidden Shift+drag=scale gesture because discoverable handles beat modifier keys for non-technical authors. The mobile-inheritance default (documented as `shapes.md` conv. 7) came from a concrete confusion — melt showed a "broken mobile layout the author never made," which was really an explicit-but-untuned `aspectRatio 9/16`; the fix is a default (mobile == desktop until diverged) plus a state chip so divergence is never silent. Plan: `…_scene-nav-builder-v2-plan.md`. |
| 2026-07-21 | **Shapes share an authoring/resolution contract, documented once in `shapes.md` → "Authoring & resolution conventions": dual wiki+markdown link syntax; basename-first resolution; a shape resolves its own refs when the general pass skips them (raw is source-of-truth, resolution is a render-time projection); literal spaces authored / encoding is a render concern; a shared tri-state + `on/off/true/false` boolean vocabulary; builders emit authorable markdown preserving the authored target.** | These conventions kept re-emerging in scene-nav builder work and apply to every ref-carrying shape (garden too), so they belong in the shape SSOT, not per-visualizer. The raw-is-source-of-truth rule is what lets a builder round-trip the exact grammar (and keeps `.md` links clickable in Obsidian); the shared boolean vocabulary prevents author syntax errors across shapes. Grammar realized first in scene-nav v2.1 (`hoverGlow`/`hoverScale`/`label:false` tri-state, `[[wiki]]` goto). Spec: `docs/implementation-plans/2026-07-21_scene-nav-builder-rework-and-resolution.md`; Plan 1: `…_scene-nav-grammar-v2.1-plan.md`. |
| 2026-07-20 | **A magic machine whose output is a shape's fence IS that shape's builder — it lives in the shape's `builder/` folder and the standalone magic machine is deleted. First case: `scene-nav-builder` → `lib/visualizers/scene-nav/builder/`, served as a debug-mode overlay (`debug: on` → lazy ESM bundle), with the Shopify embed export reusing the shape's own renderer.** | Two implementations of one format (the magic machine's parser/generator vs. the shape's) had already drifted and would keep drifting — the consolidation makes the shape's `parser.js`/`renderer.js` the single source of truth for both authoring and rendering. In-context editing (drag on the *real* rendered scene with real images/CSS) beats the standalone's upload-images workflow for scene composition. The builder ships per-site with the build, so there is no new serving infrastructure; a standalone wrapper page and the builder marketplace stay deferred (a `debug: on` block on any — even unlisted — page is a shareable playground). Scene-nav v2 grammar decisions (nested bullets, `goto:`, md-link alt text) live in `lib/visualizers/scene-nav/schema.md` + the 2026-07-20 spec. |
| 2026-07-20 | **Onboarding docs are split into two audience buckets — `docs/onboarding/` (public, stranger-safe, travels with every fork) and `docs/collaborators/` (the collaborator kit) — and they LINK INTO the architecture SSOTs rather than restating them.** | The doc-sprawl / "fine-tune in 10 places" risk is really a single-source-of-truth problem: an onboarding doc that *re-explains* what `themes.md`/`shapes.md`/`settings-registry.md` already say creates a second copy that drifts. Rule: onboarding is a *guided path* that links to the SSOT ("set your tokens → see themes.md"), never a paste of it. The two-bucket split quarantines the "report to Leon" machinery (field log) that only makes sense for a collaborator, keeping the public path clean for a stranger who has no relationship to the maintainer. Mirrors the guides-vs-`shape-authoring-log.md` (distilled truth vs. raw derivation) pattern already trusted for shapes. Deferred (Rule of Three): no `/onboard` skill/command yet (path not written even once), and the field-log is not generalized to the public set until a *second* collaborator exists. Spec: `docs/implementation-plans/2026-07-20_onboarding-docs.md`. |
| 2026-07-20 | **Collaborator onboarding-friction is auto-logged via two deliberately-separated files: a *tracked* `<name>-field-notes.md` (reaches the maintainer via PR) driven by a *gitignored* `CLAUDE.local.md` (exists only on the collaborator's clone).** | Solves "how does *her* AI log, but not *mine*" by construction, not runtime detection: the driving instruction lives in a gitignored file that only exists on her machine, so the maintainer's Claude never sees it and never writes to her log. The log itself is committed, so it travels back. `CLAUDE.local.md` is a current, non-deprecated Claude Code "Local instructions" scope that auto-loads every session (verified against the docs 2026-07-20), so logging is automatic with no `@import` and no external-import approval dialog. Logging is a *judgment* call ("was that friction?"), so it is an AI instruction, **not** a hook (hooks are for deterministic lifecycle events). The maintainer owns the *template* (`docs/collaborators/CLAUDE.local.template.md`); the collaborator's copy is a one-time personal seed. |
| 2026-07-20 | **External-forker deploy model = B1: the forker BUILDS in their own GitHub Actions, then deploys to the maintainer's Cloudflare. Shared deploy workflows are kept inert in forks via an `if: github.repository == 'LSanten/bloob-haus-webapp'` guard on every job.** | Leon wants the processing + logs on the forker's side (she sees her own runs) while owning the hosting himself (his Cloudflare will consolidate all subdomains soon; saves her needing her own CF account). Accepted secrets tradeoff: her fork holds a Cloudflare API token for Leon's account, and CF Pages tokens are **account-scoped** (Pages:Edit reaches all his projects) — mitigated by a dedicated, revocable token rotated when her collaborator access ends. The workflow guard is about **noise, not security**: a fork can't deploy Leon's sites anyway (secrets never copy to forks, so the workflows would just fail), but without the guard they fail *loudly* on every push; the guard skips them cleanly in any fork while leaving Leon's own deploys untouched (condition true in his repo). Her deploy lives in a fork-only `deploy-odalys.yml` (template in `docs/collaborators/`), mirroring the AE `deploy-alter-engineers.yml` pattern. Vault→fork rebuild trigger is a `repository_dispatch`, not a CF Deploy Hook. |
| 2026-07-20 | **`_bloob-shapes.md` is the canonical identity/shape registry (key column `bloob-shape`); the reader accepts it ahead of legacy `_bloob-types.md` / `_bloob-objects.md` (first found wins). `publish-filter.js` excludes any `_bloob-*` file by prefix, not an enumerated list.** | Completes the bloob-shape unification (TECH-DEBT #34): one forward-facing key drives layout *and* identity (via `resolveIdentityKey`, precedence `bloob-type` > `bloob-object` > `bloob-shape` so legacy vaults are untouched). Reading legacy filenames keeps marbles/buffbaby working with zero migration. The publish-filter was generalized to the whole `_bloob-*` **reserved system-file prefix** because enumerating names had already missed `_bloob-auto-tagging.md` (leaking as a page on melt *and* marbles) — same bug class, so fix it once at the right level; `_index.md` is explicitly not matched. Per-shape `fastcomments`/`showvisitorcount` *gating* was left deferred (columns parsed, templates don't consume them) — no current vault needs it and it's a Multi-Site template change. |
| 2026-07-07 | **Bloob Haus owns a dedicated free Gmail (`dev.bloob@gmail.com`) as the single owner identity for Google Cloud (OAuth app) + Scaleway; `admin@bloob.haus` is a Cloudflare Email Routing forward into that inbox; never tie OAuth-app/cloud-billing to a personal account. Upgrade to Workspace only at public launch if a branded consent-screen email is wanted.** | Ownership/handoff/blast-radius: the account that owns the OAuth app + cloud billing must be handoff-able and not shared-fate with personal email — but Workspace ($ ) isn't justified yet. A free Gmail gives a dedicated project inbox cleanly separated from personal; `admin@bloob.haus` (Cloudflare forward, *not* a Google account) is the public/branded address that lands in that inbox. A branded consent-screen support email would require the account to *be* `admin@bloob.haus` (loses the dedicated inbox) or a Workspace Group (paid) — and it's the *smallest* trust lever anyway (consent-screen trust = app name + verification + verified `bloob.haus` domain + minimal scopes, not the support email). So the branded email is deferred to a deliberate pre-launch verification/branding bundle. Same Gmail used for Scaleway; 2FA + recovery phone/email set on it. |
| 2026-07-07 | **Subdomain auto-provisioning (`username.bloob.haus`) moves EARLY — a first-milestone spike feature, not V2.** New work it pulls forward: automated **Scaleway↔Cloudflare DNS/API wiring** (hostname + Worker route + cert issued on signup). Path-based URLs are the fallback. | Subdomains turned out *cheap* (own-zone wildcard DNS + Worker routing, no per-hostname fee — already noted in the phase-3 doc), and a haus that's live-at-signup is a high-value onboarding "wow." The wiring is fiddly (DNS propagation, route registration, certs), so prototype it early rather than discovering it late. Reverses the phase-3 doc's "subdomains deferred to V2." See phase-3 doc → "Scope change". |
| 2026-07-07 | **Postgres stays Scaleway Managed Postgres (confirmed); frontend is JS/TS end-to-end; no Python in V1.** | Serverless Postgres (Neon) has EU regions but is a US company → CLOUD Act attaches to the company, reintroducing the exposure we're avoiding; Scaleway is EU-jurisdiction + co-located with the app. JS/TS everywhere = one language DB→API→UI, shared service layer, full reuse of the Eleventy/visualizer/shapes ecosystem. A second runtime (Python) is only justified by a Python-only library, and then only as its *own* isolated Serverless Function behind HTTP — never co-mingled. "Fully serverless" = serverless *containers*/jobs/Worker; Postgres is the one always-on piece; cold starts accepted for now. |
| 2026-07-07 | **`_bloob-shapes.md` = registry (file); `_bloob-shapes/` = definitions folder. Two authoring paths (Claude Code / webapp upload) produce the identical `_bloob-shapes/<name>/` structure.** | A file and a folder can coexist by stem. The registry (`_bloob-types.md` successor) maps shape→metadata; the folder holds shape *code* (manifest, schema.md, renderer, styles). Identical structure across both authoring paths is what makes a shape port cleanly between Repo Mode and the webapp (backend stores the webapp upload in the same Object-Storage layout). Standardized on `_bloob-shapes/` over the conceptual doc's `/bloob-haus-shapes/` to match the `_bloob-*` convention. See `docs/architecture/shapes.md` → "User-authored shapes". |
| 2026-07-07 | **User-authored code (shapes, magic machines, apps) runs client-side only; build-time renderers reserved for our own / approved shapes; approval via public GitHub PR; third-party & reused shapes are runtime-only (`browser.js`).** | The one hard line that keeps "reuse anyone's shape/app" a *review + provenance* problem instead of a *sandbox untrusted server code* problem. Client-side-only collapses the technical blast radius (no infra/secret/DB/other-user access); residual risk is abuse/reputation/legal (e.g. a client-side miner steals a visitor's CPU), addressed by public-PR review + CSP + ToS, not sandboxing. ken-burns/scene-nav are already client-side. Ledger records author+approver+visibility (provenance). Full build-time-code sandboxing = future TODO. |
| 2026-07-08 | **Scaleway account rebuilt from scratch under the same `dev.bloob@gmail.com` identity, billing country set to US at signup; the original account (created with country = Germany, stuck behind a support ticket to change it) is abandoned.** | Scaleway apparently requires a support ticket to *change* an already-set billing country but not to *choose* one at signup — a fresh account with the right country from the start bypassed the stuck ticket entirely. No new Google identity was needed: Scaleway account creation doesn't require a Google login, so the still-open "consolidate onto one long-term dedicated Gmail (candidate: `bloob.haus@gmail.com`)" cleanup is decoupled and deferred — Scaleway's account email is changeable later (Settings → User information → Edit) without recreating the account, so it isn't blocking. |
| 2026-07-07 | **Write the canonical `schema.md` template EARLY (Phase 2), and serve shape schemas at stable URLs for AI/MCP.** | `schema.md` is the human- + AI-readable shape contract; getting it consistent before 20 shapes accrete divergent formats is the single highest-leverage cheap task for the whole ecosystem (a template, not code). Serving schemas at `shapes.bloob.haus/<shape>/schema.md` lets AIs crawl them and lets a future MCP ("how to create a shape") be a thin wrapper — the truth stays in `schema.md`. Resolves shapes.md open Q3. |
| 2026-07-06 | **Phase 3 webapp backend lives on Scaleway EU from day one; identity via Better Auth; content as markdown files + Postgres ledger; Cloudflare Worker fronts a public/private split; API-first.** (Design only — not yet built. See `phases/phase-3/2026-07-06_webapp-backend-identity-architecture.md`.) | Privacy-first: with **0 backend infra built** and a hardening stance on US-CLOUD-Act exposure, building the user system on Scaleway *from the start* avoids a painful Cloudflare→Scaleway migration later (Leon's central open question). **One Next.js app** (not split into API+frontend) = fewest moving parts for a solo dev; splitting is a scale-time optimization deferred by Rule of Three (no second API consumer until the Phase-4 MCP server). **Better Auth** supersedes the May vision plan's NextAuth: as of 2026 Lucia is deprecated and Auth.js/NextAuth is frozen to security patches; Better Auth manages its own schema and is framework-agnostic. Content stays **flat markdown in object storage** (not a DB) to preserve "your notes are files, export anytime." **Public content stays on Cloudflare** (already public → free/fast/DDoS-protected); only **private content needs an EU origin**, reached via a Worker that *proxies, never caches* (Cloudflare bills CPU time, not I/O wait, so proxying is ~free). **API-first**: the UI only ever calls the API, so anything a human can do an agent/MCP can too — MCP is a thin Phase-4 wrapper needing only dual auth (session cookie OR API key) + a callable service layer, both baked in now. |
| 2026-07-06 | **Cross-origin embed auto-height via a namespaced postMessage protocol.** A framed page broadcasts `{__bloobEmbed:'height', height, path}` to its parent; parents apply heights only from trusted `*.bloob.haus` origins, matched to the right iframe by `contentWindow === e.source`. Both roles live in `themes/_base/partials/iframe-resize.njk` under the existing `features.iframe_resize` flag | The prior `resizeIframe` only reads same-origin `contentDocument`, so one subdomain embedding another (`leons`↔`buffbaby`) silently never resized. postMessage is the only channel across the origin boundary. Namespacing (`__bloobEmbed`) avoids reacting to unrelated widget postMessage traffic; the `*.bloob.haus` allowlist + height clamp (0–20000) + source-window match keep a hostile framed page from doing more than nudging layout. Reused the existing flag rather than adding one. Foreign iframes (YouTube) don't speak the protocol and are out of scope — their sizing is a separate aspect-ratio-CSS concern. Companion: a `font` embed URL param (mirrors `bg`/`text` in `embed.njk`) lets an embedder match the host page's font; kept as a URL param, not a registry setting, since it's embedder-chosen presentation. |
| 2026-07-03 | **Comments are a shape behavior** — a shape decides whether/where comments render by including `partials/comments.njk` in its `layout.njk`; commentability resolves through explicit shape → vault `default_shape` → `page.njk` fallback → per-page `comments:false` | Matches the ontology's `commentable?` behavior (ontology.md Q1). Placement belongs to the shape (content-bottom, a slot, etc.), and index/utility shapes (folder-preview, rss-feed) simply omit the partial. Include-in-layout is the mechanism for now; a `commentable` flag in each shape's `manifest.json` is the future formalization (see the bloob-shapes-unification plan). The partial self-guards (no fence or `comments:false` → no-op), so including it is always safe. |
| 2026-07-03 | Site-wide snippets (analytics/embeds) are authored as **raw named code fences in the `_bloob-settings.md` body**, not structured frontmatter | The user authors by pasting a whole `<script>`, and a raw `<script>` in YAML frontmatter breaks gray-matter. One primitive — "inject this raw block at head/bodyEnd on every page" — covers GoatCounter, FastComments, and arbitrary embeds; named fences (`goat-counter-tracking`, `header-snippet`, …) are friendly aliases over two buckets. An **empty fence means off** ("present means on"), so no extra enable flag is needed. See `2026-07-03_snippet-injection-analytics.md`. |
| 2026-07-03 | Per-page tokens inside snippets (`{{ page_id }}`, `{{ page_url }}`, `{{ page_title }}`) are substituted by a **scoped `injectPageVars` filter**, never by re-rendering pasted HTML through Nunjucks | Snippets live once but render per-page (FastComments needs the page filename as `urlId`, matching the old Jekyll `page.file_name` + File Identity Convention). Running arbitrary pasted HTML back through the template engine is a footgun — a stray `{{` or `{%` would crash the build. A filter that replaces only a fixed vocabulary is predictable and safe while still being idiomatic Eleventy. |
| 2026-07-03 | GoatCounter gets **no analytics-specific code** in the shared partials; embed (`/slug/embed/`) versions are tracked under their natural `/…/embed/` path | An earlier version added a `window.goatcounter.path` override to group embed hits under `/embed/…`. Removed it: embeds are already served at `/slug/embed/` so GoatCounter records them distinctly by default. "It's just a normal embed" — keeping the generic snippet partials free of GoatCounter strings is worth more than the minor dashboard-grouping nicety. Re-adding the override later is a ~4-line change. |
