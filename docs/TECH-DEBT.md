...
> and would restore this file's value. New items added since this flag (#35–38, #40–41) are current.
...
| 4 | Per-page visualizer activation not implemented | Medium | Every page loaded every visualizer's CSS + JS. | ✅ **Done 2026-07-27.** Shipped behind `features.per_page_visualizers`. Shapes opt in via `detect` in manifest.json; unannotated shapes still load everywhere. Enabled + audited on melt (41 pages, 644 drops) and marbles (1159 pages, 17601 drops); pages went from 24 visualizer assets to 11–13. **buffbaby and alter-engineers still OFF** — audit each with `node scripts/audit-visualizer-detection.js --src=src-<name>` before enabling. See `docs/architecture/visualizers.md` → "Per-page asset loading". |
...
| 25 | `watch-themes.js`: theme hot-reload silently fails on Windows | Low | `import.meta.url === \`file://${process.argv[1]}\`` check fails on Windows paths (backslash vs forward slash); `assemble-src.js` standalone entry never runs; reassembly does nothing; dev server must be restarted to pick up theme file changes | ✅ RESOLVED 2026-07-20 (S59) — added `scripts/utils/is-main.js` (`isMainModule()`, uses `pathToFileURL`, tested) and replaced the buggy idiom in all 13 scripts. Direct-run now works cross-platform. |
...
| 40 | Content-generating shapes not yet on the pure-renderer standard | **High** | `folder-preview`, `circular-nav`, `fridge-magnets`, `tags`, `graph` build their markup in `browser.js`, so **a crawler sees nothing** and the future standalone playground cannot render them without a second implementation (the exact drift that killed the scene-nav magic machine). `collection` was fixed 2026-07-27 and is the reference. | Convert each when next touched — extract a pure `renderer.js`, leave `browser.js` behavior-only, add the per-variant `<a href>` test. Do NOT retrofit speculatively. See `docs/architecture/visualizers.md` → "The pure-renderer standard". | ⬜ Open |
| 41 | `collection` and `folder-preview` collide on unscoped `fp-*` CSS rules | Low (was High) | Both stylesheets declare `.fp-card`, `.fp-cards`, `.fp-marble(s)`, `.fp-bubble(s)`, `.fp-field`, `.fp-seo-wrapper`, `.fp-search-input`, `.fp-filter-placeholder`. `folder-preview.css` loads later (alphabetical) so it wins **every** tie — collection's stylesheet is largely dead code today. `.fp-card`/`.fp-cards` genuinely differ (folder-preview adds border-radius, card-bg, border, overflow, transition), so **AE's cards are currently styled by folder-preview**. This is a live blocker for #4: per-page loading changes which stylesheet is present and would silently restyle AE. | Before shipping #4, either namespace collection's rules under `.collection-visualizer` (and port the richer `.fp-card` body so AE is unchanged — verify against AE), or extract the shared `fp-*` card/marble CSS into one owner both shapes import. ✅ **Resolved 2026-07-27** for the collection side: measured in a real browser across 5 display modes and 4 themes, only THREE rules actually diverged (`.fp-card__title`, `.folder-preview__link`, `.folder-preview__icon`) — the "11 colliding rules" figure came from a faulty text-diff. Those three are now scoped, preserving AE's current rendering exactly, and `tests/css-independence.test.js` guards it. **Remaining (low):** the reverse direction is unverified — whether `folder-preview` depends on `collection.css` — and the duplicate class names are still a latent trap. Extend the independence test to the reverse direction when folder-preview is retrofitted under #40. | 🟡 Partly done |
...
| 39 | Scene-nav builder v2 bugs + no real-browser test (unmerged branch `scene-nav-builder-rework`) | Medium | Blocks merging the builder. **Rotation grip reverts on release** (browser.js hover handlers clobber `el.style.transform` on mouseleave — root cause confirmed); **marquee rubber-band select is buggy** (fragile event model). No builder drag interaction has been verified in a real browser (headless can't drag) — only the pure selection/transform math is unit-tested. | Next-round plan `docs/implementation-plans/2026-07-22_melt-builder-next-round.md`: fix B1 (hover gate) + B2 (overlay marquee rewrite via `frontend-design`), add undo/redo, do a real-browser pass, then merge. | ⬜ Open |
