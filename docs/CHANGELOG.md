...
**Worked on:** Decoupling SEO from the visualization layer — `collection` refactored to a new **pure-renderer standard**; melt's Resources page swapped `folder-preview` → `collection`. Suite 637 → **668 tests**. Spec `docs/superpowers/specs/2026-07-27-collection-pure-renderer-design.md`.

- **The bug, measured:** melt's live Resources page served a crawler **0 links** (`folder-preview` shipped an empty div; JS built everything). After: **3 crawlable `<a href>`**, marbles visually unchanged, drag physics intact — verified in a real headless browser, not asserted.
- **Root cause:** `collection` emitted build-time HTML only for `display: cards`, because `render-card.js` was its only pure renderer; `list`/`slider`/`bubbles`/`marbles` lived as `createElement` code inside `browser.js`'s IIFE. **"Has SEO" literally meant "has an isomorphic renderer."** Investigation showed all four were plain static markup with behavior bolted on — the JS-only-ness was incidental. The fix is **conformance to a standard 10 of 24 shapes already follow**, not new architecture.
- **The standard (`docs/architecture/visualizers.md` → "The pure-renderer standard"):** `parser.js` (pure) · **`resolve.js`** (pure, external data *injected*) · `renderer.js` (pure, complete markup) · `browser.js` (**behavior only**). Three hosts — Node build, future standalone playground, browser fallback — differ only in the data source. The requirement is a *pure renderer*, not that every shape run in the browser. Exempt: `latex`, `citations`, `checkbox-tracker`, `page-preview` (enhance text, generate nothing). No `seo:` toggle — `visibility: unlisted` is the real privacy tool.
- **Collection:** `core.js` → `resolve.js`, `render-card.js` → `renderer.js` (all 5 modes), `index.js` = host, `browser.js` stripped to physics/Swiper/search. **File-scope now gets build-time SEO too** — `transform()` fills the container `renderFilescope()` left empty, since it runs in Eleventy where `graph.json` exists. `eleventy.config.js` passes `pageUrl` to every visualizer transform (additive) so a collection never lists its own page.
- **AE protection without the AE vault:** the vault isn't on this machine, so `display: cards` output is frozen in a committed **golden master** (11 render cases, byte-exact) plus a guard test proving the fixture is actually read. Green after refactor. Handoff: `docs/collaborators/2026-07-27_AE-verification-seed.md`.
- **Search surfaces now share a `--sv-*` token contract** (not markup — one is Pagefind's element, one is ours). New `--sv-input-bg` / `--sv-input-bg-focus` / `--sv-input-focus-ring` are **consumed with fallbacks, never declared** in visualizer CSS, because theme `main.css` loads *before* visualizer CSS — which is exactly why melt and marbles-pouch had accumulated `!important`. Both themes' `!important` blocks removed and verified **pixel-identical** in-browser; melt's two search surfaces now match on all 7 measured properties at rest *and* on focus.
- **Melt:** `Resources/_index.md` → `​```collection source: folder=resources display: marbles`. Dropped the separate `​```search` fence (collection's own filter replaces it; the filter now narrows the marbles, which the site-wide bar never did). **Gotcha:** graph.json `section` is lowercase `resources`, not `Resources` — `folder=Resources` silently renders empty. Also added `.collection-visualizer` to melt's B4 column-width rule, else the page spills full-width.
- **New debt found:** **#41 (High)** — `collection` and `folder-preview` collide on **11 unscoped `fp-*` rules**; `folder-preview.css` loads later and wins every tie, so **AE's cards are currently styled by folder-preview** and collection's stylesheet is largely dead code. This now **blocks #4 (per-page loading)**: dropping a stylesheet would silently restyle AE. Only `.fp-search-input` scoped so far. **#40 (High)** — retrofit `folder-preview`, `circular-nav`, `fridge-magnets`, `tags`, `graph` to the standard.
- **Tooling:** `@playwright/test` pinned to **1.61.1** to match `bloob-haus-cloud/e2e` and reuse the machine-global browser cache `~/Library/Caches/ms-playwright` (chromium-1228) — no download. Unblocks the drag-interaction checks the melt builder plan keeps deferring as "headless can't drag".
- **Part 2 — per-page visualizer loading (TECH-DEBT #4) — SHIPPED same session.** Pages now load only the visualizer CSS/JS they use, behind `features.per_page_visualizers`. **melt: 610 CSS + 448 JS requests skipped across 39 pages. marbles: 17,640 CSS + 12,771 JS across 1162 pages.** Pages drop from 24 visualizer assets to 11–13.
  - **First unblocked #41.** Measured in a real browser across 5 display modes × 4 themes: only **three** rules actually diverged (`.fp-card__title`, `.folder-preview__link`, `.folder-preview__icon`) — the "11 colliding rules, cards differ" figure came from a faulty text-diff. Scoped those three, preserving AE's current rendering exactly (its `.fp-card__title` is a *merge* of folder-preview's two competing declarations). `tests/css-independence.test.js` guards it; verified the test fails on the pre-fix state before trusting it.
  - **Detection reads rendered HTML**, not source — so one mechanism covers fences, `:::`, `bloob-shape:` and theme partials. Themes emit `<!--bloob:visualizer-css-->` markers a transform fills. **Opt-in per shape** (`detect` in manifest.json); unannotated shapes always load. 20 shapes annotated, 4 marked `detect.always` with reasons, 1 unannotated.
  - **The audit is the real deliverable.** Checking "does my shape emit this class?" is NOT enough — **a stylesheet can own classes something else renders.** `article/styles.css` owns `.share-bar`, which a *shared theme partial* puts on ordinary pages; detecting only `.article-page` silently stripped it from 33 melt pages, and screenshots showed only an unexplained 5px height change. `scripts/audit-visualizer-detection.js` disables exactly what detection drops and compares element geometry. Two refinements made it truthful: strip zero-size nodes (a dropped shape's JS goes with its CSS, so its injected `display:none` scaffolding can't appear) and take a **control measurement** to exclude rAF-animated elements (marble float otherwise reads as a regression). Groups pages by detection signature → 1159 pages audit in seconds. Negative-tested against the reintroduced share-bar bug.
  - **Screenshot diffing proved too noisy to be the verdict**: crawling the *same* build twice left 12/41 pages self-differing (marble float, lazy images, external embeds). The audit is the authoritative check — melt 41/41 safe, marbles 11/11 signatures safe covering 1159 pages.
  - **buffbaby and alter-engineers left OFF** — AE can't be built on this machine, and enabling blind defeats the flag's purpose. Procedure documented in the AE seed.
