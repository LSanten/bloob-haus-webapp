...
export function transform(html) {
  ...
  let result = html.replace(sectionPattern, (match, before, rawBase64, after, innerHtml) => {
    ...
    // Pre-resolution source (if captured) → the debug blob, so the builder round-trips
    // the authored refs (note.md / [[wiki]]) instead of resolved URLs. Rendering itself
    // still uses `data` (resolved) so data-value/navigation stay correct.
    let debugScene;
    const srcMatch = (before + after).match(/data-vis-raw-source="([^"]+)"/);
    if (srcMatch) {
      try {
        debugScene = parse(Buffer.from(srcMatch[1], "base64").toString("utf-8"));
      } catch { /* fall back to `data` for the blob */ }
    }

    return render(data, { resolveImage, ...(debugScene ? { debugScene } : {}) });
  });
