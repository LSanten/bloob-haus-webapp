...
describe('scene-nav builder — selection helpers', () => {
  const els = [
    { x: 10, y: 10, scale: 10 }, // center 15,15
    { x: 40, y: 40, scale: 20 }, // center 50,50
    { x: 80, y: 80, scale: 10 }, // center 85,85
  ];
  it('elementsInRect selects by element center', () => {
    expect(elementsInRect(els, { x: 0, y: 0, w: 60, h: 60 })).toEqual([0, 1]);
    expect(elementsInRect(els, { x: 70, y: 70, w: 30, h: 30 })).toEqual([2]);
    expect(elementsInRect(els, { x: 90, y: 90, w: 5, h: 5 })).toEqual([]);
  });
  it('rangeSelect returns an inclusive, clamped, order-independent range', () => {
    expect(rangeSelect(5, 1, 3)).toEqual([1, 2, 3]);
    expect(rangeSelect(5, 3, 1)).toEqual([1, 2, 3]);
    expect(rangeSelect(3, 0, 9)).toEqual([0, 1, 2]);
  });
  it('toggle adds/removes an index without mutating', () => {
    const a = new Set([1]);
    const b = toggle(a, 2);
  ...
  it('mobileState: no mobile → not diverged', () => {
    expect(mobileState({ aspectRatio: '16/9', elements: els })).toEqual({ diverged: false, overrides: 0 });
  });
  it('mobileState: mobile aspectRatio equal to desktop → same', () => {
    expect(mobileState({ aspectRatio: '16/9', mobile: { aspectRatio: '16/9' }, elements: els }).diverged).toBe(false);
  });
  it('mobileState: different mobile aspectRatio → diverged', () => {
    expect(mobileState({ aspectRatio: '16/9', mobile: { aspectRatio: '9/16' }, elements: els }).diverged).toBe(true);
  });
  it('mobileState: per-element overrides counted', () => {
    const s = { aspectRatio: '16/9', mobile: { aspectRatio: null }, backgrounds: [],
      elements: [{ x: 1, y: 1, scale: 1, mobileOverride: { x: 5 } }, { x: 2, y: 2, scale: 1, mobileOverride: null }] };
    expect(mobileState(s)).toEqual({ diverged: true, overrides: 1 });
  });
});

describe('scene-nav builder — group transform math', () => {
  const els = [{ x: 0, y: 0, scale: 10 }, { x: 40, y: 0, scale: 10 }]; // centers 5,5 and 45,5
  it('groupBBox unions footprints and centers', () => {
    const b = groupBBox(els, [0, 1]);
    expect(b).toMatchObject({ x: 0, y: 0, w: 50, h: 10, cx: 25, cy: 5 });
  });
  it('scaleGroup(2) doubles size and spread about the group center', () => {
    const r = scaleGroup(els, [0, 1], 2);
    // group center is (25,5); element0 center 5,5 → moves to -15,5, scale 20 → x=-25
    expect(r[0]).toMatchObject({ scale: 20 });
    expect(r[0].x).toBeCloseTo(-25); // center 5 → 25+(5-25)*2 = -15, x = -15-10 = -25
    expect(r[1].x).toBeCloseTo(55);  // center 45 → 25+(45-25)*2 = 65, x = 65-10 = 55
  });
  it('moveGroup shifts all by the delta', () => {
    expect(moveGroup(els, [0, 1], 5, -3)).toEqual([{ i: 0, x: 5, y: -3 }, { i: 1, x: 45, y: -3 }]);
  });
  it('rotateGroupPositions(90) maps right-of-center to below-center', () => {
    // element1 center (45,5) is right of group center (25,5); +90° (cw, y down) → below center
    const r = rotateGroupPositions(els, [0, 1], 90);
    const el1 = r.find((p) => p.i === 1);
    const cx = el1.x + els[1].scale / 2;
    const cy = el1.y + els[1].scale / 2;
    expect(cx).toBeCloseTo(25); // moved onto the vertical through center
    expect(cy).toBeCloseTo(25); // 20 below center (which was at y=5)
  });
});
