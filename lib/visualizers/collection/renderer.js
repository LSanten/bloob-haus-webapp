  ...
  const items = pages
    .map((node) => {
      ...
      const icon = node.bloobIcon
        ...
        : "";
      return `<li class="folder-preview__item"><a href="${href}" class="folder-preview__link"${external}>${icon}<span>${esc(node.title || node.id)}</span></a></li>`;
    })
    .join("\n");
  ...
  const items = pages
    .map((node, i) => {
      ...
      const typeHtml = node.content_type
        ...
        : "";
      return `<a href="${escAttr(node.id)}" class="fp-bubble" style="width:${size}px;height:${size}px;${marginTop}">${typeHtml}<span class="fp-bubble__title">${esc(node.title || node.id)}</span></a>`;
    })
    .join("\n");
...
export function renderMarblesHtml(pages) {
  ...
  const items = pages
    .map((node, i) => {
      ...
      return `<a href="${escAttr(node.id)}" class="fp-marble" style="width:${size}px;height:${size}px;transition:none;${marginTop}"><img src="/assets/objects/marble.png" class="fp-marble__img" alt="" aria-hidden="true"><span class="fp-marble__title">${esc(node.title || node.id)}</span></a>`;
    })
    .join("\n");
