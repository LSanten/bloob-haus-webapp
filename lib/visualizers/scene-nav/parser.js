 ...
 *   \t- newTab: on                ← optional; absent = auto (same tab on-site, new tab off-site)
...
export function parse(rawText) {
  ...
  for (const el of opened) {
    ...
    elements.push({
      ...
      ...(p.overlayColor ? { overlayColor: String(p.overlayColor).trim() } : {}),
      ...(p.overlayStrength != null ? { overlayStrength: Number(p.overlayStrength) } : {}),
      action,
      value,
      // Tri-state: null = auto (same tab if the target is on this site, new tab if not).
      // Only an explicit `newTab: on/off` overrides that; see link-target.js.
      newTab: p.newTab !== undefined ? booleanKeyword(p.newTab) : null,
...
export function serializeBlock(sceneData) {
  ...
  for (const el of d.elements || []) {
    ...
    if (el.overlayColor) lines.push(`\t- overlayColor: ${el.overlayColor}`);
    if (el.overlayStrength != null) lines.push(`\t- overlayStrength: ${el.overlayStrength}`);
    ...
    // Only an explicit choice is written back — auto stays absent, so the markdown
    // carries no setting until the author actually made one.
    if (el.newTab === true) lines.push("\t- newTab: on");
    else if (el.newTab === false) lines.push("\t- newTab: off");
