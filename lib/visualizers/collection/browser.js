...
import { resolvePages, normalizeUrl } from "./resolve.js";
...
(function () {
  ...
  // NOTE: this deliberately does NOT pass filters to Pagefind.
  //
  // It used to search with `{ filters: { section: <folder> } }`, which requires
  // the site to have registered a `section` filter in its Pagefind index. melt
  // registers none (`pagefind.filters()` returns {}), so the filtered query
  // matched zero results on every keystroke and the full-text half of "combined
  // search" silently did nothing — while the metadata half kept working, which
  // is why it looked like search worked but only over titles.
  //
  // Scoping by filter was only ever an optimisation: the collection already
  // knows exactly which pages it rendered. Searching unfiltered and
  // intersecting against its own hrefs gives the same scoping with no
  // dependency on per-site Pagefind filter configuration.
  ...
  function attachCombinedSearch(container, settings) {
    ...
    input.addEventListener("input", function () {
      ...
      initPagefind().then(function (pf) {
        ...
        return pf.search(q).then(function (results) {
          if (pending !== q) return;
          return Promise.all(results.results.map(function (r) { return r.data(); }))
            .then(function (data) {
              if (pending !== q) return;
              // Normalize both sides: graph.json ids are slugified lowercase,
              // Pagefind indexes the real output path (which preserves case
              // under melt's permalink strategy). Comparing raw missed every
              // match.
              var pfUrls = new Set(data.map(function (d) { return normalizeUrl(d.url); }));
              // Union: show if metadata OR Pagefind matched — never hide more than step 1 did.
              // Intersecting against this collection's own hrefs is what scopes
              // the full-text results to the collection.
              items.forEach(function (el) {
                el.hidden = !metaMatches.has(el) && !pfUrls.has(normalizeUrl(itemHref(el)));
              });
            });
        });
      });
