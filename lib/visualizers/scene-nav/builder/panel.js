...
export function initPanel(root, scene, { onClose } = {}) {
  ...
  function applyOverlayToDom(i) {
    ...
    const props = ["--snb-overlay", "--snb-overlay-mask", "--snb-overlay-opacity", "--snb-overlay-blend", "--snb-overlay-filter"];
    ...
    node.style.setProperty("--snb-overlay-filter", o.filter);
  }
  ...
  function wireCanvas() {
    ...
    cleanups.push(mountMarquee(container(), {
      isEmptyTarget: (e) => e.target === container() || e.target.classList.contains("scene-nav-bg-clip"),
  ...
  function renderPanel() {
    ...
    if (idx.length === 1 && primary != null) {
      ...
      if (el.overlay) {
        const cur = (el.overlayColor || "#ffffff").toLowerCase();
        const OV_COLORS = [["#ffffff", "bright"], ["#3f3f3f", "dark grey"], ["#000000", "black"]];
        kids.push(h("div", { class: "snb-row" }, [h("label", {}, ["color"]),
          h("div", { class: "snb-swatches", style: "flex:1" }, OV_COLORS.map(([c, t]) =>
            h("div", { class: `snb-sw${cur === c ? " snb-sel" : ""}`, style: `background:${c};color:${c}`, title: t, onclick: () => { el.overlayColor = c; applyOverlayToDom(primary); renderPanel(); } })
          ).concat([h("input", { type: "color", value: el.overlayColor || "#ffffff", style: "width:24px;height:22px;padding:1px;background:transparent;border:1px solid #3c2f5c;border-radius:4px;cursor:pointer", oninput: (e) => { el.overlayColor = e.target.value; applyOverlayToDom(primary); } })]))]));
        const sVal = el.overlayStrength != null ? el.overlayStrength : 70;
        const sRange = h("input", { type: "range", min: 0, max: 100, step: 5, value: sVal });
        const sNum = h("input", { class: "snb-num", type: "number", min: 0, max: 100, step: 5, value: sVal });
        const setS = (v) => { el.overlayStrength = Number(v); sRange.value = v; sNum.value = v; applyOverlayToDom(primary); };
        sRange.addEventListener("input", (e) => setS(e.target.value));
        sNum.addEventListener("input", (e) => setS(e.target.value));
        kids.push(h("div", { class: "snb-row" }, [h("label", {}, ["strength"]), sRange, sNum]));
