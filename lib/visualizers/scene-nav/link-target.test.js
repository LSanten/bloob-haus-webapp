...
describe("clicking a scene-nav element in a browser", () => {
  ...
  async function click(extra) {
    ...
    // A `noopener` popup is severed from its opener, so it arrives as a new page on the
    // CONTEXT — page.on("popup") never sees it. URLs are read after settling, because a
    // page fires this event while still on about:blank.
    ...
    await page.locator(".scene-nav-el").click();
    await page.waitForTimeout(400);
    const url = page.url();
    const openedTabs = [];
    for (const p of opened) {
      try { await p.waitForLoadState("domcontentloaded", { timeout: 2000 }); } catch { /* blank tab */ }
      if (p.url() && p.url() !== "about:blank") openedTabs.push(p.url());
    }
    await page.close();
    return { navigatedTo: url, openedTabs };
  }

  it("navigates in the SAME tab for a link on this site", async () => {
    if (!browser || !bundle) return;
    const r = await click("");
    expect(r.openedTabs).toEqual([]);
    expect(r.navigatedTo).toContain("/contact-us/");
  }, 30_000);

  it("opens a NEW tab when the author forced newTab: on", async () => {
    if (!browser || !bundle) return;
    const r = await click("\t- newTab: on");
    expect(r.openedTabs.length).toBe(1);
    expect(r.openedTabs[0]).toContain("/contact-us/");
  }, 30_000);
});
