...
| 4 | Per-page visualizer activation not implemented | Medium | Every page loads every visualizer's CSS + JS. **Blocked by #41** — dropping a stylesheet changes which of the colliding `fp-*` rules wins, which would silently restyle AE's cards. Resolve #41 first. | Part 2 of the 2026-07-27 collection work; see `docs/superpowers/specs/2026-07-27-collection-pure-renderer-design.md` | ⬜ Open |
...
| 41 | `collection` and `folder-preview` collide on 11 unscoped `fp-*` CSS rules | **High** | Both stylesheets declare `.fp-card`, `.fp-cards`, `.fp-marble(s)`, `.fp-bubble(s)`, `.fp-field`, `.fp-seo-wrapper`, `.fp-search-input`, `.fp-filter-placeholder`. `folder-preview.css` loads later (alphabetical) so it wins **every** tie — collection's stylesheet is largely dead code today. `.fp-card`/`.fp-cards` genuinely differ (folder-preview adds border-radius, card-bg, border, overflow, transition), so **AE's cards are currently styled by folder-preview**. This is a live blocker for #4: per-page loading changes which stylesheet is present and would silently restyle AE. | Before shipping #4, either namespace collection's rules under `.collection-visualizer` (and port the richer `.fp-card` body so AE is unchanged — verify against AE), or extract the shared `fp-*` card/marble CSS into one owner both shapes import. Only `.fp-search-input` is scoped so far (2026-07-27). | ⬜ Open |
