...
function ensureIconStyle() {
  ...
  s.textContent =
    ...
    ".snb-edit-icon.snb-editing{background:#2e6b4f;border-color:#57c78f}";
...
export default function initBuilder(root) {
  ...
  function setEditing(on) {
    icon.classList.toggle("snb-editing", on);
    icon.textContent = on ? "● Editing — close" : "✎ Edit scene";
  }

  icon.addEventListener("click", (e) => {
    e.preventDefault();
    const editingThis = activePanel && activePanel.__root === root;
    if (activePanel) activePanel.close(); // closes any open panel (fires its onClose → resets its pill)
    if (editingThis) return; // clicking the active scene's pill again toggles it closed
    setEditing(true);
    activePanel = initPanel(root, scene, {
      onClose: () => { activePanel = null; setEditing(false); },
    });
    activePanel.__root = root;
  });
}
