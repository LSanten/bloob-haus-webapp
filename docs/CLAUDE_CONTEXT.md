...
- **`article` is the canonical page/post shape** (consolidated 2026-07-27) — a layout-only shape that ships its own `layout.njk`, so it works in every theme. Carries title/subtitle/banner/author/byline, **both date-pill conventions** (comma form *and* separate-key + list form; latest list entry wins — `scripts/utils/date-field.js`), share bar, comments, opt-in visitor count, responsive media embeds (YouTube/Vimeo 16:9), tables. **The share bar's CSS is NOT article's** — it lives in `themes/_base/assets/css/base.css` (2026-07-28), because the markup is a `_base` partial that marbles-pouch's `page.njk` and `_base/pages/embed-pages.njk` also include; mechanics in base.css, skin in each theme's `main.css`. Tuned per theme via `--article-*` tokens. **melt runs entirely on it**; its own `page.njk` is now just an alias. `bloob-shape: page` is NOT a shape — prefer `article`.
- **Nested shapes govern their own presentation.** A `preserve` container (like `article`) must let a nested shape style itself, but CSS has no notion of a shape boundary, so a container's prose rules leak in and win by specificity accident. Convention: a nested shape scopes its rules to its own container root. Guarded by `tests/shape-nesting.test.js`. Open decision: cascade layers vs `@scope` — see `shapes.md`.
...
- Test suite: 909 tests across 51 files (Vitest), co-located visualizer tests
