...
### Session 71 — August 15, 2026
**Worked on:** two link/encoding bugs on melt, both rooted in percent-encoding never being decoded. Suite 851 → **899 tests**.

- **Markdown-style transclusions with spaces in the filename now expand.** `![Bios](Bios%20for%20whitney%20and%20vicki.md)` rendered a "Could not embed" placeholder while `![[Bios for whitney and vicki]]` worked. The preprocessor rewrote markdown-style embeds to wiki form but never percent-decoded the target, so every file-index lookup missed — including the strip-non-alphanumerics fallback, which turned the target into `bios20for20whitney20and20vicki` (exactly the broken slug on the live site). It only fired when a filename contained a space, which is why it looked intermittent: `photo-grid-face-massage.md` always worked, and `About.md` had one of each.
- Fixing it in the **transclusion handler** rather than at the call site closed two further gaps: **nested markdown-style embeds inside an embedded page were never normalized at all** (the recursion fed on raw `rawBody`), and `![alt](page.md#Heading)` was not recognized unlike `![[page#heading]]`. External `.md` URLs are now left alone instead of becoming broken vault embeds.
- **A literal `%` in a filename crashed the build.** `decodeURIComponent` throws `URIError: URI malformed` on `50% off flyer.png`; scene-nav's `parser.js` and `resolve.js` both called it unguarded, aborting the whole build with an error naming neither the file nor the shape. Routed through a new pure `safeDecode`.
- **The scene-nav builder now emits refs that resolve in Obsidian.** "Copy ::: block" wrote literal spaces, and a raw space terminates a CommonMark link destination — so a pasted block showed broken links in the vault while the site built fine. Image refs are percent-encoded on the way out; the parser already decodes on the way in, so the round-trip is symmetric and stable. `goto:` keeps its verbatim guarantee **except** for spaces in a markdown-link target; **wiki-links are never re-encoded** (`[[a note]]` is Obsidian syntax where spaces are legal, and `%20` would make it hunt for a file literally named `a%20note`).
- **This contradicted a documented cross-shape rule**, found only by reading the SHAPE TRINITY. `shapes.md` #4 said builders emit literal spaces. Its reasoning was sound about **storage** (a decoded model keeps basename resolution encoding-agnostic — kept) but carried the same form over to **emission** without arguing for it. Amended to "stored decoded, emitted encoded"; #6's verbatim-`goto` guarantee narrowed by the one exception; the July plan's superseded lines marked rather than deleted.
- **Verified the site output does not change** — melt's seven rendered `data-value` targets and eight `<img src>` values are byte-identical before and after, including after rewriting `_index.md` itself. Only the builder's clipboard export differs. Melt's seven real `goto`s round-trip byte-for-byte.
- **melt vault:** `_index.md`'s scene-nav block rewritten to the encoded form (model asserted identical before writing), plus Leon's own renames — `An evening with MELT.md` → `articles/an evening with melt.md` and a new `structure of a monthly bodywork skillshare.md`.
- **Then garden fixed too, and it was worse than the debt row said.** The attribute tokenizer had **no quote awareness at all**, so any value with a space truncated at the first one — and since `serializeElement` *writes* `bg:"my backdrop.png"`, **garden's own round-trip silently corrupted data** (read back as `"my`). `schema.md` had documented quoting since day one; it only ever worked because hex colors contain no spaces. One tokenizer fix repairs `src:` and `bg:` together, and garden now follows convention #4 like scene-nav — stored decoded, emitted encoded, with `rootPath` encoding per segment so the `<img src>` is a valid URL. An unquoted space stays ambiguous in a space-delimited grammar and can't be supported, but it is **no longer silent**: the grammar has exactly three flags, so any other bare token warns and names the fix.
- **`safeDecode`/`encodeRef` moved to `lib/visualizers/_utils/ref-encoding.js`** — the existing shared-code home (`bg-color.js` already serves five shapes) — so scene-nav and garden share one implementation instead of a third copy. Verified inlined into the browser bundle. No live vault uses `garden` yet, so the grammar change carried zero migration risk. Suite → **921**.

- **`_utils/` named as an actual pattern, and the convention made real** (2026-08-16). Shared pure helpers for shapes now have a documented home, a bar for entry (a *shared contract*, not merely shared code), and a stated cost: `shapes.md` calls a shape "a folder you can zip, share, fork", and **a shape importing `_utils/` is no longer that** — which matters for the user-authored/marketplace direction, where `_utils` would become a platform API surface. Written up in `visualizers.md` while the folder is small enough to decide cheaply.
- **Found while documenting it: the underscore convention was fiction.** `bundle-visualizers.js` enumerated `_utils` as a visualizer, so it sat in `visualizers.json` as a manifest-less entry — which that same doc defines as **always loaded**, i.e. a phantom shape that also skews `audit-visualizer-detection.js`. The bundler now skips underscore-prefixed folders (25 shapes, was 26). I had asserted the exclusion in the doc *before* checking; verifying the claim is what surfaced the bug.
- **Also surfaced: `ken-burns-zoom` ships no `manifest.json`** (TECH-DEBT #48) — a real shape that therefore always loads its assets, silently opting out of per-page loading. Left unfixed (needs a `detect` audit, not a guess) but **pinned** by `tests/build/visualizer-discovery.test.js`, which fails if a *second* shape loses its manifest. Suite → **924**.- **Then fixed holistically, because it was bigger than scene-nav.** `markdown-link-resolver.js` and `attachment-resolver.js` were confirmed to throw identically, so a `%` in *any* filename killed *any* build. **All 13 unguarded `decodeURIComponent` call sites across 10 files** now route through one `scripts/utils/safe-decode.js`; `transclusion-handler.js`'s local copy was folded into it. `lib/visualizers/scene-nav/decode.js` deliberately keeps its own — it is browser-bundled and must not import from `scripts/`.
- **Why this was safe to do in one pass:** `safeDecode` is a *pure widening* — for every input that did not throw, it returns byte-identical output, so only the crash path can change. Verified rather than asserted: 909 tests green, all three sites preprocess clean (melt 17 / marbles 471 / buffbaby 67 files, zero errors), melt's homepage byte-identical, and a purpose-built vault containing `50% off flyer.png` now builds with **0 broken links** and resolves the image to `/media/50%25%20off%20flyer.png` where it previously died on `URIError`.
- **Caught by the suite, worth remembering:** the scripted import insertion put a line *inside* a multi-line `import { … }` in `preprocess-content.js`, breaking the module. `npm test` failed the whole file rather than a single assertion — a scripted multi-file edit needs a syntax check per file, which is now part of the verification (`node -e "import('./file.js')"` across every touched file).

### Session 70 — July 28, 2026
**Worked on:** Leon's melt dev-notes todo list (2026-07-28 batch), start to finish, while he was away. Suite 808 → **851 tests**.

- **scene-nav no longer spawns a tab for a link to its own site.** Every `goto` did `window.open(…, "_blank")`, so the melt homepage nav opened a new tab for an internal page. The tab is now decided by **origin** — same tab on-site, new tab off-site — with an explicit tri-state `newTab: on/off` overriding it and writing nothing to the markdown while it stays on auto. Policy is a pure `link-target.js` with the origin **injected**, so the runtime, the builder's live preview and the tests cannot disagree. Two edges decided deliberately: `mailto:`/`tel:` stay in-tab (the OS handler takes over), and a scheme-less `example.com` is indistinguishable from a relative path, so it routes as internal and the **builder says so** rather than guessing. Exported embeds invert to new-tab — they sit on someone else's page. Builder GUI gained an auto/this-tab/new-tab control with a live "what auto does" chip, in both single and multi-select. Verified in headless Chromium against the bundled runtime.
- **Share bar is icon-first across every theme.** "Share Copy link Embed" spelled out under each article collapsed to a row of overlapping circles that reveal their label on hover, or on the **first tap** on touch (second tap acts). The confirmation state forces the label open so "Copied!" is never hidden. Two near-duplicate copies deleted rather than kept in sync — **melt's `gap: 10px` silently cancelled the shape's negative margin**, rendering the buttons 1px apart instead of stacked, which is exactly the bug that copy existed to cause; the shape now sets `gap: 0` explicitly. The share button is `.remove()`d rather than hidden when `navigator.share` is absent, because a hidden-but-present button keeps its `:first-child` slot.
- **Date pill is frosted glass by default for every theme** — colors untouched. The substantive change is what the accent mixes *with*: `color-mix(… transparent)` instead of an opaque `--bg-color`. `--article-date-tint` / `--article-date-blur` restore the flat chip.
- **melt `{.button}` contrast fixed, root cause found.** It was not a color choice — `a.button` and `.article-body a` have **identical specificity (0,1,1)**, and `article/styles.css` loads after `main.css`, so when melt's pages moved onto the `article` shape the prose-link color won and the CTA became purple-on-purple. Every rule is now qualified at (0,2,1) so it wins on specificity, never file order. Restyled to glass with deep-plum text.
- **Visitor count (opt-in) from GoatCounter's JSON endpoint** — rendered into our own markup, so **no image badge, iframe or branding**. Answers Leon's open question: the counter **is** gated behind a per-site GoatCounter admin switch ("Allow using the visitor counter"), which **defaults to off** and returns `403 Need to enable the 'allow using the visitor counter' setting` — confirmed against the live API for both `melt` and `studiobloob`. Because the build cannot detect that, the widget renders `hidden` and unhides only when a count arrives. Site code is **derived** from the existing `goat-counter-tracking` snippet rather than configured twice.
- **Per-page `visitor_count:` overrides the site default both ways**, mirroring `comments:`. The comments half of that todo turned out to be **already true** — `article/layout.njk` already includes `partials/comments.njk` and the partial already self-guards on the vault having a `fast-comments-embed` fence — so it was verified and locked with tests rather than rebuilt.
- **Verified in a real browser, not by eye:** melt (share bar geometry, hover/tap expansion, "Copied!", glass values, date pill, button contrast, visitor count on both a live 403 and a mocked 200) and **marbles** (share bar inherits correctly and keeps its own purple tokens). `audit-visualizer-detection.js` re-run for melt after the markup change: 8/8 signatures safe across 44 pages.
- **Share-bar CSS moved from the `article` shape to `themes/_base/assets/css/base.css`** (same session, after the above). The markup is a `_base` partial that three unrelated hosts include — article's layout, marbles-pouch's `page.njk`, `_base/pages/embed-pages.njk` — so a page can carry a share bar without being an article, and owning the CSS in a shape meant it was per-page gated on that shape's `detect`. `.share-bar` had been added to article's `detect.selectors` purely to paper over the mismatch; **it is now removed**, because base.css loads unconditionally and there is nothing left to gate. Verified on a marbles page with `articleCssLoaded: false` and the bar fully styled in marbles' own purple. Second win: melt's overrides dropped from `.article-page .share-bar .share-btn` (0,3,0) back to plain `.share-bar .share-btn` — base.css loads *before* `main.css`, so a theme now wins on source order instead of needing a compound selector to outrank a shape stylesheet that loads after it. Split is **mechanics in base.css, skin in each theme's main.css**.
- **Copy link is now the first share button on every page and theme.** It is the only button guaranteed to exist (the share button is removed at runtime when `navigator.share` is missing, i.e. every desktop), so leading with it makes the bar start with the same icon everywhere *and* makes `:first-child` — which cancels the overlap's negative margin — always land on a visible button.
- **melt footer:** two-line credit (Leon's copy change) got `line-height: 1.45` on `.melt-footer__credit`; the footer's own 1.8 is tuned for separate single-line rows and read as a gap mid-sentence. Invalid `</br>` corrected to `<br>`.
- **Environment gotcha worth knowing:** `_site` is **not cleaned between sites**. Building melt and then marbles left marbles pages being served with melt's `main.css`, which silently invalidated a cross-site check — the marbles page reported melt's glass values. `rm -rf _site` before switching sites, and treat any cross-site visual comparison that skipped that as void.
- **New debt:** TECH-DEBT #45 — marbles-pouch's `.marble-date` is a second date-pill implementation that did not get the glass treatment, because marbles does not use the `article` shape.
- **Not done / needs Leon:** (1) **`melt-website` is 1 commit ahead of origin** — `show_visitor_count: true` is committed but `git push` hung on the macOS keychain, and the counter stays invisible on the live site until it lands. The GoatCounter switch itself is now **on** (endpoint returns site total 87, `/about-melt/` 8). (2) The handwritten **"All Possible Settings"** tables in `bloob-haus-marbles` and `buffbaby`'s `_bloob-settings.md` need `show_visitor_count` / `visitor_count` added by hand — not done here, since those are content repos this session doesn't commit. (melt's vault has no such table.)
...
- **Pure-renderer standard adopted** (`docs/architecture/visualizers.md`): `parser.js` · `resolve.js` (external data injected) · `renderer.js` (pure, complete markup) · `browser.js` (**behavior only**). Three hosts — Node build, future playground, browser fallback — differ only in data source.
- **`collection` refactored to it.** Every display mode now renders crawlable HTML at build time, not just `cards`; file-scope too. Measured on melt's Resources page: **0 → 3 crawlable links**, marbles visually unchanged, physics intact. No `seo:` toggle — `visibility: unlisted` is the real privacy tool.
- **Search surfaces share a `--sv-*` token contract.** New appearance tokens are consumed-with-fallback (never declared in visualizer CSS), which let melt + marbles-pouch drop their `!important` overrides; verified pixel-identical.
- **melt Resources swapped** `folder-preview` → `collection` (`display: marbles`), dropping its separate search fence.
- **Per-page visualizer loading shipped** (TECH-DEBT #4, now closed) behind `features.per_page_visualizers`. melt −610 CSS/−448 JS across 39 pages; marbles −17,640/−12,771 across 1,162. Pages: 24 assets → 11–13. Detection reads *rendered HTML* and is opt-in per shape.
- **`scripts/audit-visualizer-detection.js`** is what makes that safe: it caught `article/styles.css` owning `.share-bar` (rendered by a *shared theme partial*), which had silently stripped styling from 33 pages. Screenshot diffing proved too noisy to be the verdict — the same build crawled twice self-differed on 12/41 pages.
- **#41 resolved for collection**, downgraded High → Low. Only **three** rules actually diverged, not eleven — the original figure came from a faulty text-diff; a real browser disagreed.

- **Two follow-up bugs found by Leon in the browser, both fixed same session.** (a) Search set `hidden` but marbles/bubbles stayed visible — `display:flex` overrides the UA `[hidden]` rule, and `.fp-card[hidden]` already existed for exactly this reason. My original check asserted `el.hidden === true`, i.e. read back the value it had just set, so it could never fail; the new test measures rendered geometry. (b) **Full-text search never worked**: `attachCombinedSearch` filtered Pagefind by `{section: <folder>}`, but melt registers no Pagefind filters (`pagefind.filters()` → `{}`), so the query matched zero on every keystroke while the metadata half kept working — pre-existing since `5615104`, but exposed when melt's site-wide `search` fence was dropped. Now searches unfiltered and intersects against the collection's own hrefs (with URL normalization — graph.json slugifies to lowercase, Pagefind preserves case). `data-fp-tags` added to marbles/bubbles/list so tag matching works there too.

**Part 3 — melt onto the shared `article` shape (same session):**
- **Root cause of the weird Resources page found:** `bloob-shape: page` resolves to *nothing* — there is no `lib/visualizers/page/`, so the layout came entirely from the step-5 fallback, which is `page.njk` for ordinary pages but **`base.njk` for index files**. `about-melt` worked by coincidence. Worse, the preprocessor strips the leading `# heading` expecting `page.njk` to re-render it, so the index page lost **both** its column and its title. One cause, both symptoms.
- **`article` upgraded** with four generic features (all themes gain them): **`date_updated` pill** (article had only `date_created` — adopting it blind would have dropped the updated date from 10 of 23 melt pages), `banner_text`, `show_date_created`/`show_date_updated` settings, and **media embeds** — article had *no* iframe handling at all; YouTube/Vimeo now verified at 632×356, ratio 1.778, no overflow. Plus tables, em/strong, heading anchors. New pure `scripts/utils/date-field.js` supports **both** date conventions (comma form for marbles/AE, separate-key + list form for melt; latest list entry wins).
- **melt migrated:** all 15 content pages now declare `bloob-shape: article`. `/about-melt/` measured before and after — width 680, left 300, padding `60px 24px 120px`, body 16.8px, title 57.6px: **identical**. Colours/fonts needed zero work (article already reads the standard token contract); only 6 sizing tokens. **Resources is fully fixed** — title, column, date pill, and B4's prose-clipping resolved.
- **Nesting bug found + fixed:** marble titles came out underlined — `.article-body a` (0,1,1) beats `.fp-marble` (0,1,0). A container's prose stylesheet reaching into a nested shape. Fixed by scoping the nested shape to its own root, guarded by `tests/shape-nesting.test.js` (asserts both directions).
- **Cleanup:** melt's `page.njk` is now a thin alias to `article.njk` (it can't be deleted — the fallback resolves to that literal path); 22 dead `.page-*` rules removed, keeping the three still rendered by `404.njk`/`embed-pages.njk`. **`settings-registry.md` brought current** — registered 10 new article tokens, the 3 per-page article settings, both date conventions, `features.per_page_visualizers`, collection's `placeholder`, and S66's `background_image` override (that last one had been unregistered since S66).
- **melt share bar → glassmorphism** (melt-only override, tuned to Leon's dimmer reference: 0.1 fill, blur(7px)).

**Docs / standards:**
- **CLAUDE.md now requires THE SHAPE TRINITY** — `ontology.md` + `shapes.md` + `visualizers.md`, all three, always, before any shape work, with a table of what each owns and a definition of what counts as shape work. Written because the rule was violated this session: two core shapes were reworked from `visualizers.md` alone.
- **`shapes.md`:** the unknown-shape fallback section was **wrong** (said `page.njk`; it's `base.njk` for index files) — corrected with the melt case. New section on **presentation containment**: styling follows the existing container-contents policy (preserve → child styles itself; override → container imposes), the problem being that CSS has no notion of a shape boundary. Records cascade-layers vs `@scope` as an open decision. `page` added to the shape status table as **Not a shape**.

**Left incomplete:** buffbaby + alter-engineers have per-page loading **OFF** (AE can't be built on this machine — audit first, see `docs/collaborators/2026-07-27_AE-verification-seed.md`). AE's collection search bar changes square → pill and needs Leon's call. #41's reverse direction (does `folder-preview` depend on `collection.css`?) unverified. #40 retrofit of the other content-generating shapes untouched. melt vault edit unpushed (Leon's).
...
- **Grammar v2.1 (Plan 1, TDD):** per-element **`hoverGlow`/`hoverScale`** toggles (default on; parse → `data-hover-*` → `browser.js` `hoverStyles()` → serialize); **`label` tri-state** (`label: false`/`off` suppresses; value overrides; absent = alt); **`[[wiki]]` goto**; shared **`on/off/true/false`** vocabulary. `browser.js` DOM wiring guarded behind `typeof document` (unit-testable pure `hoverStyles`).
- **Image resolution (Plan 3a):** scene-nav resolves its OWN image refs (it writes `[alt](x.png)` links the general resolver skips) via the attachment index — basename-first, path-aware, verbatim fallback → root-relative encoded URLs. Fixed the melt homepage **Contact us 404** (basename ref) and made scene-navs work on subpages (**also fixed marbles core-family**, whose `../media/…` refs 404'd on its subpath). Raw refs stay in the debug blob (round-trip). `preprocess` now writes `_data/attachmentIndex.json` (generic build data for ref-carrying shapes).
- **Builder GUI rework (Plan 2):** debug-overlay side panel reworked — foldable sections (Scene/Add/Elements/Selected/Background/Export) + **hide/reopen** toggle; ported bloob-relevant controls from the deleted magic-machine GUI (glow-color **swatches** + picker, **flip H/V**, glowIntensity + x/y/scale/rotation sliders, **show-on**); new **hover-glow / hover-enlarge / show-label** toggles; **mockup image upload** (local object-URL preview + "not saved to your vault" notice; backend save deferred). Shopify UI removed (`embed-serializer.js` kept in code, silent). Export = **Copy ::: block** only.
...
- **Comments as a shape behavior** — a shape decides *whether* and *where* comments render by including `partials/comments.njk` in its `layout.njk`. Added the include to the `article` shape layout (article pages bypass `page.njk`, so they'd otherwise be skipped); `page.njk` stays commentable as the default/fallback. Commentability resolves through explicit shape → vault `default_shape` → `page.njk` fallback → per-page `comments: false`. Documented in `shapes.md` ("Comments — a shape behavior", incl. the fallback chain). Verified by temporarily shaping a melt page as `article` (comments rendered, then reverted).
