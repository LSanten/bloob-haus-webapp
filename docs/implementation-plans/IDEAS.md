...
- [x] **Bloob-shapes unification (`_bloob-types.md` → `_bloob-shapes.md`)** — DONE 2026-07-20: reader `_bloob-shapes.md` + `bloob-shape`-column support (legacy files still read), the `_bloob-*` publish-filter fix, melt as a clean reference, the `_base` scaffold, and the doc reconciliation all shipped. Plan (now in `_completed/`): [`_completed/2026-07-03_bloob-shapes-unification.md`](_completed/2026-07-03_bloob-shapes-unification.md).
  - [ ] **Deferred follow-up — per-shape behavior gating (steps 3–4).** `fastcomments` / `showvisitorcount` are now *parsed* into the registry, but templates don't *consume* them yet: `partials/comments.njk` isn't gated on the shape's `fastcomments`, and GoatCounter stays site-wide. A template-only change when wanted (registry data is already exposed); touches all four themes' `page.njk` + the comments partial (Multi-Site-rule change). No current vault needs it.
