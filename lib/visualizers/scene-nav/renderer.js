...
function buildContainer(backgrounds, elements, aspectRatio, maxWidth, extraClass, edgeFade) {
  ...
  // Soft Edges: Gaussian-blur a solid rect to produce a feathered alpha mask —
  // same technique as Word's "Soft Edges" effect. The blur is on the MASK, not
  // the image, so content stays sharp while edges dissolve smoothly.
  let maskSvg = "";
  let maskStyle = "";
  if (fade > 0) {
    const uid      = Math.random().toString(36).slice(2, 8);
    const maskId   = `snav-mask-${uid}`;
    const filterId = `snav-filt-${uid}`;
    // inset  — opaque rect inset from each edge (0..1 of element size)
    // blur   — Gaussian stdDeviation in objectBoundingBox units (1 = element size)
    // blur must satisfy: inset / (sqrt(2) * blur) > 2.33 so the Gaussian
    // decays to <0.1% opacity at the element edge (no visible semi-transparent fringe)
    const inset = +(fade * 0.5).toFixed(5);
    const blur  = +(fade * 0.15).toFixed(5);
    const rw    = +(1 - 2 * inset).toFixed(5);
    maskSvg = `<svg xmlns="http://www.w3.org/2000/svg" style="position:absolute;width:0;height:0;overflow:hidden" aria-hidden="true">` +
      `<defs>` +
      `<filter id="${filterId}" x="-200%" y="-200%" width="500%" height="500%">` +
      `<feGaussianBlur stdDeviation="${blur}"/>` +
      `</filter>` +
      `<mask id="${maskId}" maskContentUnits="objectBoundingBox">` +
      `<rect x="${inset}" y="${inset}" width="${rw}" height="${rw}" fill="white" filter="url(#${filterId})"/>` +
      `</mask>` +
      `</defs>` +
      `</svg>`;
    maskStyle = `;mask:url(#${maskId});-webkit-mask:url(#${maskId})`;
  }
  ...
  return `${maskSvg}<div class="${cls}" style="${containerStyle}">
${bgHtml}
${elsHtml}
</div>`;
}
