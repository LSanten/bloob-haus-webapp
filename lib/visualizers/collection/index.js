...
import { renderCollectionInner } from "./renderer.js";
...
function buildContainer(settingsJson, inner = "") {
...
export function renderFilescope(settings, _body) {
  return buildContainer(JSON.stringify(settings || {}));
}
...
export function transform(html, opts = {}) {
  ...
  let result = html.replace(codeBlockPattern, (_match, rawSettings) => {
    ...
    if (inner === null) {
      ...
      return buildContainer(settingsJson);
    }

    return buildContainer(settingsJson, inner);
  });
  ...
  const emptyContainerPattern =
    /<div class="collection-visualizer" data-pagefind-ignore data-collection-settings='([^']*)'><\/div>/gi;
